# MessageDump (md) - copy last command and output to clipboard
...
function global:_md_main {
    ...
    switch ($Command) {
        "on" { _md_on; return }
        "off" { _md_off; return }
        "status" { _md_status; return }
        "update" { _md_update; return }
        "uninstall" { _md_uninstall; return }
        ...
        { $_ -in "help", "-h", "--help" } { _md_help; return }
        "" {
            if (-not (_md_is_enabled)) {
...
if (-not $script:_MD_INIT) {
    ...
    Start-Job -ScriptBlock {
        param($updateCheckPath, $rawUrl, $version)
    ...
    } -ArgumentList @($script:MD_UPDATE_CHECK, $script:MD_RAW_URL, $script:MD_VERSION) | Out-Null
}
...
Export-ModuleMember -Function '_md_main' -Alias $script:MD_CMD_NAME
